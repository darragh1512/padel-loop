-- supabase/migrations/20260809000000_consensus_entries.sql
-- CONSENSUS SCORESHEETS: replaces the old "one player submits, the rest tap
-- Confirm" flow with independent entry + automatic comparison:
--   1. The game OWNER starts the match (start_match_result) — this creates the
--      pending result and fixes the 2v2 teams.
--   2. EVERY participant enters their own set scores (submit_result_entry),
--      stored per-player in the new match_result_entries table.
--   3. When the last scoresheet lands, the RPC compares them. All identical →
--      the canonical sets are written to match_result_sets and the result is
--      finalised ('confirmed' + winning_team). Any difference → 'disputed';
--      everyone sees the mismatch and can resubmit, which re-runs the compare.
-- RUN in SQL editor (real change) AND save in repo.

-- 1. Constraints on the existing tables ---------------------------------------
-- `team` already exists (see the baseline) — it was just never constrained.
alter table public.match_result_participants
  add constraint match_result_participants_team_check check (team in (1, 2));

-- 'disputed' joins the lifecycle; nothing else is a legal status.
...
-- every write now goes through the two RPCs above.
