/* Brand-signature SVG components: the dashed loop arc ending in the ball.
   Recoloured to the token palette — sage lines on soft sage/bone grounds,
   flat and editorial, no gradients. */
  ...
  return (
    <div className="mt-4 h-38 rounded-card relative overflow-hidden border border-line bg-accent-soft">
      <svg viewBox="0 0 346 150" fill="none" className="absolute inset-0 w-full h-full">
        ...
        <line x1="106" y1="18" x2="106" y2="132" stroke="var(--color-accent)" strokeOpacity=".22" strokeWidth="1" />
        <line x1="240" y1="18" x2="240" y2="132" stroke="var(--color-accent)" strokeOpacity=".22" strokeWidth="1" />
        <line x1="38" y1="75" x2="106" y2="75" stroke="var(--color-accent)" strokeOpacity=".22" strokeWidth="1" />
        <line x1="240" y1="75" x2="308" y2="75" stroke="var(--color-accent)" strokeOpacity=".22" strokeWidth="1" />
        <path
          ...
          strokeLinecap="round"
        />
        <circle cx="180" cy="36" r="8" fill="var(--color-surface)" />
        <path d="M173 34 Q180 30 187 34" stroke="var(--color-accent)" strokeWidth="1.2" opacity=".5" />
        <path d="M173 38 Q180 42 187 38" stroke="var(--color-accent)" strokeWidth="1.2" opacity=".5" />
      </svg>
    </div>
  );
}
...
export function LoopRing({
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg viewBox="0 0 74 74" className="absolute inset-0 w-full h-full text-accent">
        <path
...
export function MiniLoop({ size = 34 }: { size?: number }) {
  return (
    <svg viewBox="0 0 34 34" fill="none" className="text-accent" style={{ width: size, height: size }}>
      <path
        ...
        strokeLinecap="round"
      />
      <circle cx="5.5" cy="10" r="3.5" fill="currentColor" />
    </svg>
  );
}

/* The "padel loop" wordmark + tagline — Instrument Serif lowercase in ink,
   tagline in secondary sans. Used on the auth screens. */
