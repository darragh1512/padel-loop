...
export default function CreateGamePage() {
  ...
  return (
    <main className="px-5 pt-6 relative">
      ...
      <label className="pl-surface w-full rounded-field px-4 py-3 mb-2.5 flex justify-between items-center gap-3 focus-within:border-accent">
        <span className="text-label text-ink-secondary shrink-0">Venue</span>
        <input
          type="text"
          value={venue}
          onChange={(e) => setVenue(e.target.value)}
          placeholder="e.g. Malahide Padel Club"
          className="flex-1 min-w-0 bg-transparent text-right text-body font-medium text-ink outline-none placeholder:text-ink-faint placeholder:font-normal"
        />
      </label>

      <label className="pl-surface w-full rounded-field px-4 py-3 mb-2.5 flex justify-between items-center gap-3 focus-within:border-accent">
        <span className="text-label text-ink-secondary shrink-0">Location</span>
        <input
          type="text"
          value={location}
      ...
      <label className="pl-surface w-full rounded-field px-4 py-3 mb-2.5 flex justify-between items-center gap-3 focus-within:border-accent">
        <span className="text-label text-ink-secondary shrink-0">Date</span>
        <input
          type="date"
          value={date}
          onChange={(e) => setDate(e.target.value)}
          className="flex-1 min-w-0 bg-transparent text-right text-body font-medium text-ink outline-none [color-scheme:light]"
        />
      </label>

      <label className="pl-surface w-full rounded-field px-4 py-3 mb-2.5 flex justify-between items-center gap-3 focus-within:border-accent">
        <span className="text-label text-ink-secondary shrink-0">Time</span>
        <input
          type="time"
          value={time}
      ...
      <div className="pl-surface rounded-field p-4 mb-2.5">
        <div className="flex justify-between text-label text-ink-secondary mb-3">
        ...
        <div className="h-1 bg-line rounded relative">
          ...
          <div className="absolute top-1/2 left-[28%] w-4.5 h-4.5 rounded-full bg-surface border border-line -translate-x-1/2 -translate-y-1/2" />
          <div className="absolute top-1/2 left-[68%] w-4.5 h-4.5 rounded-full bg-surface border border-line -translate-x-1/2 -translate-y-1/2" />
        </div>
      </div>
      ...
      >
        <div>
          <div className="text-body font-medium text-ink">Open to the Loop</div>
          <div className="text-label text-ink-secondary mt-0.5">
            Anyone in range can request a slot
          </div>
        </div>
        <span
          className={`w-11.5 h-6.75 rounded-full relative shrink-0 transition-colors duration-150 ease-out ${
            openToLoop ? "bg-accent" : "bg-line"
          }`}
        >
          <span
            className={`absolute top-[3px] w-5.25 h-5.25 rounded-full bg-surface transition-all duration-150 ease-out ${
      ...
      <div className="rounded-card p-4 my-3.5 border border-line bg-accent-soft">
        <div className="flex justify-between text-label text-ink-secondary py-0.5">
          <span>Court fee</span>
        ...
        <div className="flex justify-between items-baseline text-label text-ink-secondary py-0.5">
